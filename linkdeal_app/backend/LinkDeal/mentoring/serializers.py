...
class RecentReviewsSerializer(serializers.ModelSerializer):
    """Serializer for recent reviews in dashboard."""
    
    mentee_name = serializers.CharField(source='mentee.full_name', read_only=True)
    mentee_initials = serializers.SerializerMethodField()
    mentee_picture = serializers.SerializerMethodField()
    time_ago = serializers.SerializerMethodField()
    
    class Meta:
        model = Review
        fields = ['id', 'mentee_name', 'mentee_initials', 'mentee_picture', 'rating', 'comment', 'time_ago']
        
    def get_mentee_initials(self, obj):
        name = obj.mentee.full_name
        return "".join([part[0] for part in name.split()]).upper()[:2]
    
    def get_mentee_picture(self, obj):
        request = self.context.get('request')
        # Check profile picture first
        if obj.mentee.profile_picture and request:
            return request.build_absolute_uri(obj.mentee.profile_picture.url)
        # Fallback to social picture
        if obj.mentee.social_picture_url:
            return obj.mentee.social_picture_url
        return None
...
class MentorForFavoriteSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'profile_picture_url', 'skills', 'session_rate', 'average_rating', 'reviews_count'
        ]
...
class RelationMenteeSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'country', 'profile_picture_url', 'social_picture_url'
        ]
...
class MentorPublicListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'profile_picture_url', 'social_picture_url', 'bio', 'skills', 'languages',
            'session_rate', 'average_rating', 'reviews_count', 'categories', 'is_favorite'
        ]
    ...
    def get_average_rating(self, obj):
        from scheduling.models import Session
        avg = Session.objects.filter(
            mentor=obj, rating__isnull=False
        ).aggregate(avg=Avg('rating'))['avg']
        return round(avg, 1) if avg else None
    
    def get_reviews_count(self, obj):
        from scheduling.models import Session
        return Session.objects.filter(mentor=obj, rating__isnull=False).count()
...
class MentorPublicDetailSerializer(MentorPublicListSerializer):
    ...
    def get_recent_reviews(self, obj):
        ...
        sessions = Session.objects.filter(
            mentor=obj,
            rating__isnull=False
        ).select_related('mentee').order_by('-scheduled_at')[:5]
        ...
        for session in sessions:
            reviews.append({
                ...
                'mentee': {'full_name': session.mentee.full_name},
                'created_at': session.updated_at.isoformat() if session.updated_at else session.scheduled_at.isoformat(),
            })
