...
interface FinancialSettingsProps {
  onSettingsChange?: (settings: { platformFee: string; minSessionPrice: string }) => void;
}

const FinancialSettings: FunctionComponent<FinancialSettingsProps> = ({ onSettingsChange }) => {
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      try {
        ...
        setPlatformFee(response.data.platform_fee_percentage || '10');
        setMinSessionPrice(response.data.min_session_price || '50');
      } catch (error) {
  ...
  useEffect(() => {
    if (onSettingsChange) {
      onSettingsChange({ platformFee, minSessionPrice });
    }
  }, [platformFee, minSessionPrice, onSettingsChange]);
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <div className="absolute right-3 top-1/2 -translate-y-1/2 flex flex-col gap-1">
                <button
                  type="button"
                  onClick={() => setMinSessionPrice(String(parseFloat(minSessionPrice || '0') + 10))}
                ...
                <button
                  type="button"
                  onClick={() => setMinSessionPrice(String(Math.max(0, parseFloat(minSessionPrice || '0') - 10)))}
                  className="w-4 h-4 rounded bg-purple-500/20 hover:bg-purple-500/30 transition-colors flex items-center justify-center group"
                >
                  <svg width="8" height="8" viewBox="0 0 8 8" fill="none" xmlns="http://www.w3.org/2000/svg">
