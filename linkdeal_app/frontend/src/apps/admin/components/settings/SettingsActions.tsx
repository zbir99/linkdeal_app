...
const SettingsActions: FunctionComponent<SettingsActionsProps> = ({ onSave, isSaving = false }) => {
  ...
  return (
    <div className="w-full h-12 relative flex items-start gap-4 text-center text-sm font-arimo">
      ...
      <div
        className={`h-12 flex-1 relative shadow-[0px_10px_15px_-3px_rgba(112,_8,_231,_0.5),_0px_4px_6px_-4px_rgba(112,_8,_231,_0.5)] rounded-xl text-white cursor-pointer hover:bg-[#8020F7] hover:scale-105 hover:shadow-xl transition-all duration-200 flex items-center justify-center gap-2 ${isSaving ? 'opacity-70 cursor-wait hover:scale-100' : ''}`}
      ...
      >
        ...
        <div className="leading-5">{isSaving ? 'Saving...' : 'Save All Changes'}</div>
      </div>
    </div>
  );
};
