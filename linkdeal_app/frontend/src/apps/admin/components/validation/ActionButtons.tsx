...
const ActionButtons: FunctionComponent<ActionButtonsProps> = ({ onApprove, onReject, approveLoading = false }) => {
  return (
    <div className="rounded-2xl border border-white/10 bg-white/5 p-4 sm:p-6 backdrop-blur-md">
      ...
      <div className="space-y-3">
        ...
        <button
          ...
          className={`w-full h-11 rounded-xl bg-green-600 hover:bg-green-700 text-white font-medium transition-all duration-300 hover:scale-[1.02] hover:shadow-lg hover:shadow-green-500/20 flex items-center justify-center gap-3 text-sm sm:text-base ${approveLoading ? 'opacity-50 cursor-wait hover:scale-100' : ''}`}
        >
          {approveLoading ? (
          ...
          <span className="truncate">{approveLoading ? 'Approving...' : 'Approve Mentor'}</span>
        </button>
