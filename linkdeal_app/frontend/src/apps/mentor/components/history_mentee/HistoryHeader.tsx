...
export const HistoryHeader: FunctionComponent<HistoryHeaderProps> = ({ onBack, menteeName }) => {
  return (
    <div className="mb-4 sm:mb-6 md:mb-8">
      ...
      >
        ...
        <span className="hidden sm:inline">Back to Mentees</span>
        <span className="sm:hidden">Back</span>
      </button>
      
      <div className="space-y-1 sm:space-y-2">
        <h1 className="text-2xl sm:text-3xl md:text-4xl font-semibold text-white">
          {menteeName ? `${menteeName}'s History` : 'Mentee History'}
        </h1>
        <p className="text-white/60 text-sm sm:text-base md:text-lg">View all past sessions and feedback</p>
      </div>
    </div>
  );
};
