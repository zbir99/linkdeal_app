...
export const PricingActions: FunctionComponent<PricingActionsProps> = ({ onSave, isSaving = false }) => {
  ...
  return (
    <div className="flex gap-4 pt-4">
      <button
        ...
        disabled={isSaving}
        className="flex-1 h-12 px-4 rounded-xl bg-white/5 border border-white/10 text-white/70 hover:bg-white/10 hover:text-white transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
      >
        Cancel
      </button>
      ...
      <button
        onClick={handleSave}
        disabled={isSaving}
        className="flex-1 h-12 px-4 rounded-xl bg-[#7008E7] text-white hover:bg-[#6007c5] hover:scale-105 hover:shadow-lg hover:shadow-purple-500/30 transition-all duration-300 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
      >
        {isSaving ? (
          <>
